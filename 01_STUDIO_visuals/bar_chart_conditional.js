/**
 * Bar Chart (Conditional Formatting) - Universal Version
 *
 * Strategy: Try CDN sources first (fast, lightweight for Looker Core / environments
 * with network access). If ALL CDN sources fail (e.g. Looker Original on AWS with
 * strict CSP), silently fall back to the bundled inline copy of Highcharts 11.2.0.
 * No user action required — this file works in both environments automatically.
 */

// --- 1. HIGHCHARTS LOADER: CDN-first with inline fallback ---

// Inline fallback: Highcharts 11.2.0 bundled. Only executed if all CDNs fail.
const _HC_INLINE_BUNDLE = function() {
...
const _loadScript = (src) => {
  return new Promise((resolve, reject) => {
    // If already injected and loaded, resolve immediately
    ...
    script.onerror = () => { console.warn(`[Viz Loader] CDN failed: ${src}`); reject(); };
    document.head.appendChild(script);
  });
};

// Try each CDN in sequence; if all fail, run the inline bundle as a guaranteed fallback
const _hcLoadPromise = _HC_CDNS
  .reduce((chain, cdn) => chain.catch(() => _loadScript(cdn)), Promise.reject())
  .then(() => {
    console.log('[Viz Loader] Highcharts loaded from CDN.');
  })
  .catch(() => {
    console.warn('[Viz Loader] All CDN sources failed. Using bundled Highcharts fallback.');
    _HC_INLINE_BUNDLE();
    if (typeof Highcharts !== 'undefined') {
      console.log('[Viz Loader] Bundled Highcharts loaded successfully.');
    } else {
      console.error('[Viz Loader] CRITICAL: Bundled fallback also failed. Chart cannot render.');
    }
  });
...
looker.plugins.visualizations.add({
  ...
  create: function(element, config) {
    console.log("[Viz] 0. Create triggered"); // LOGGING ADDED

    // FIX FOR PDF RENDERING:
    // Force a height if the container is 0 (common in PDF generator)
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    const chartOptions = {
      ...
      chart: {
        ...
        spacing: [10, 10, 10, 10],
        // --- PDF FIX: Disable Animation when printing ---
        animation: !isPrint,
        // ------------------------------------------------
        events: {
          load: function() {
      ...
      plotOptions: {
        series: {
          ...
          cursor: 'pointer',
          // --- PDF FIX: Disable Series Animation ---
          animation: !isPrint,
          // -----------------------------------------
          point: {
            events: {
    ...
    // --- PDF FIX: Safe Done() Call with Reflow ---
    if (isPrint) {
        console.log("[Viz] Rendering for Print. Waiting 500ms before done()...");
        // Give browser render cycle time to paint chart before snapshot
        setTimeout(() => {
            if (this.chart) this.chart.reflow();
            done();
        }, 500);
