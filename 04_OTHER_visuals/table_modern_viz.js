 ...
 * Version: 4.12.5 - Logic Fixes + Enhanced Logging
 * Build: 2026-01-13
 */
  ...
  create: function(element, config) {
    element.innerHTML = `
      <style>
        ...
        .subtotal-toggle { cursor: pointer; margin-right: 8px; font-weight: bold; }
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    if (config.enable_subtotals && config.subtotal_dimension) {
      if (this.state.lastSubtotalDimension !== config.subtotal_dimension) {
        console.log('[STATE LOG] Grouping Changed:', config.subtotal_dimension);
        this.state.collapsedGroups = {};
        this.state.lastSubtotalDimension = config.subtotal_dimension;
        this.state.forceInitialCollapse = true;
        this.state.currentPage = 1; // Always reset page on dimension change
      }
    }
    ...
    console.log(`[DATA LOG] Visible Rows: ${processedData.length}, Total Pages: ${totalPages}`);
  ...
  renderTable: function(pageData, totalRows, totalPages, config, queryResponse) {
    ...
    pageData.forEach((row, i) => {
      ...
      if (config.show_row_numbers) {
        html += `<td>${(isSub||isGrand) ? '' : (this.state.currentPage - 1) * config.page_size + i + 1}</td>`;
      }
  ...
  attachListeners: function(config, totalPages) {
    ...
    this.container.querySelectorAll('.subtotal-row').forEach(row => {
      row.onclick = () => {
        ...
        console.log('[LOG] Toggled Group:', g);
    ...
    this.container.querySelectorAll('.p-btn').forEach(btn => {
      btn.onclick = () => {
        ...
        console.log('[STATE LOG] Page Changed To:', self.state.currentPage);
        self.updateAsync(self.state.data, null, config, self.queryResponse, {}, () => {});
      };
    });
  },
