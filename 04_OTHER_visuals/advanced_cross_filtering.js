looker.plugins.visualizations.add({
  ...
  label: "Single Item Cross Filter (0-origin Data Bar)",

  // ============================================================
  //  Configuration Options Definition
  //  Settings that users can modify in the Looker edit screen
  // ============================================================
  options: {
    ...
    selection_mode: {
      ...
      display: "select",
      values: [
        { "Multiple Selection (Checkbox)": "multi" },
        { "Single Selection (Radio)": "single" }
      ],
      default: "multi",
    ...
    title_override: {
      label: "Title Override (blank for field name)",
      type: "string",
      default: "",
      section: "Basic Settings",
      order: 2
    ...
    selected_measure: {
      ...
      type: "string",
      display: "select",
      values: [{ "None": "" }], // Dynamically overwritten in updateAsync
      default: "",
    ...
    bar_position: {
      ...
      display: "select",
      values: [
        { "Full Row (Full Width)": "row" },   // Display as row background
        { "Under Number (Number Only)": "number" } // Display mini bar under number
      ],
      default: "row",
      section: "Visualization Settings",
      order: 3
    ...
    measure_format: {
      label: "Measure Display Format",
      type: "string",
      default: "",
      placeholder: "#,##0", // Excel-like format specification
      section: "Visualization Settings",
      order: 6
    },
    ...
    // ============================================================
    //  Design Settings (background, font, colors, etc.)
    // ============================================================
  ...
  /**
   * Initialization: DOM element creation and basic CSS definition
   */
  create: function(element, config) {
    element.innerHTML = `
      <style>
         ...
         * Dynamic style changes achieved by injecting values from JS
         */
        .cv-container {
        ...
        /* Background layer control: Stack background image and background color using pseudo-elements */
        /* Layer 1: Background image (back-most) */
        .cv-container::before {
          content: "";
          position: absolute;
        ...
        /* Layer 2: Background color overlay (on top of image) */
        .cv-container::after {
          content: "";
          position: absolute;
          top: 0; left: 0; width: 100%; height: 100%;
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    // Add 'px' to unitless numeric strings
    ...
    // Set CSS variable to element
    const setVar = (name, val) => root.style.setProperty(name, val);

    // Apply CSS variables from config values
    ...
    // Initialize/maintain search state (to preserve text and mode during re-renders)
    if (!this.searchState) {
    ...
    // Interpolate between two colors (factor: 0.0-1.0)
    ...
    // Calculate gradient color based on value with min/mid/max colors
    ...
    // Format numeric value (comma separator, decimal places, etc.)
    ...
    try {
      // Error checking and initial validation
      ...
      // Dynamic generation of measure options
      ...
      // Data traversal and min/max value identification
      let minVal = Infinity;
      let maxVal = -Infinity;
      ...
      const modes = [
        { id: "contains", label: "Partial Match (Standard)", placeholder: "Search..." },
        { id: "exclude",  label: "Exclude Search (Not)",   placeholder: "Text to exclude..." },
        { id: "range",    label: "Numeric Range", placeholder: "e.g.: 100-500" },
        { isDivider: true },
        { id: "regex",    label: "Regular Expression (Regex)", placeholder: "Regex..." }
      ];
      ...
      items.forEach(item => {
        ...
        // Reflect crossfilter selection state
        ...
        // Draw measure value and data bar
      ...
      const handleModeSelect = (m, item) => {
          ...
          searchInput.value = "";
          this.searchState.text = ""; // Clear text when mode changes
      ...
      // Apply filtering and reflect state before initial render
      applyFilter();

      // Add to DOM tree (re-render)
      requestAnimationFrame(() => {
        root.innerHTML = "";
        root.appendChild(wrapper);
      });

      // Register event to close menu on outside click (once only)
